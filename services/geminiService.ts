...
const validationSchema = {
    type: Type.OBJECT,
    properties: {
        isValid: { type: Type.BOOLEAN, description: 'Verdadero si el texto es una pregunta de examen válida y completa, falso en caso contrario.' },
        reason: { type: Type.STRING, description: 'Una breve explicación de por qué el texto es válido o no. Si no es válido, indica qué falta (ej. "Faltan las opciones de respuesta", "No se encuentra una explicación", "El texto no parece una pregunta").' }
    },
    required: ['isValid', 'reason']
};

export const validateInputText = async (inputText: string): Promise<ValidationResult> => {
    // Basic client-side check to avoid unnecessary API calls
    if (!inputText || inputText.trim().length < 25) {
        return { isValid: false, reason: "El texto es demasiado corto para ser una pregunta de examen completa." };
    }
    
    const prompt = `
    Analiza el siguiente texto y determina si contiene todos los elementos necesarios para una tarjeta de estudio de una pregunta de examen:
    1. Un enunciado de pregunta claro.
    2. Varias opciones de respuesta (generalmente numeradas o con letras).
    3. Una indicación de la respuesta correcta o una explicación/comentario de donde se pueda deducir.
    ...
    Responde ÚNICAMENTE con un objeto JSON. Si es válido y completo, 'isValid' debe ser true y 'reason' debe ser una confirmación breve. Si NO es válido o está incompleto, 'isValid' debe ser false y 'reason' debe explicar concisamente qué falta o por qué no es válido.
    `;
    ...
    try {
        ...
        console.error("Error durante la validación del texto:", error);
        // Fallback in case the validation API call itself fails
        return { isValid: false, reason: "La IA no pudo validar la estructura del texto. Puede que el formato sea muy inusual o que haya un problema con la conexión." };
    }
};
...
const flashcardResponseSchema = {
    type: Type.OBJECT,
    properties: {
        questionStem: { type: Type.STRING, description: 'SOLAMENTE el enunciado principal de la pregunta, sin incluir las opciones de respuesta.' },
        correctOption: { type: Type.STRING, description: 'La respuesta correcta, formateada como "Opción [N] correcta ([Texto de la opción])".' },
        definition: { type: Type.STRING, description: 'Una explicación clara y concisa de por qué la respuesta es correcta.' },
        optionsAnalysis: {
            type: Type.ARRAY,
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    option: { type: Type.STRING, description: 'El texto de la opción (sin el número inicial).' },
                    verdict: { type: Type.STRING, description: 'Un emoji: "✅" para la correcta, "❌" para las incorrectas.' },
                    reason: { type: Type.STRING, description: 'Una breve explicación de por qué la opción es correcta o incorrecta.' }
                },
        ...
        analysisTables: {
            type: Type.ARRAY,
            description: 'Tablas comparativas o de clasificación extraídas o generadas a partir del comentario.',
            items: {
                type: Type.OBJECT,
                properties: {
                    title: { type: Type.STRING, description: "Título descriptivo de la tabla." },
                    headers: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Cabeceras de la tabla." },
                    rows: { type: Type.ARRAY, items: { type: Type.ARRAY, items: { type: Type.STRING } }, description: "Filas de la tabla, donde cada fila es un array de strings." }
                },
        ...
        keyConcepts: {
            type: Type.ARRAY,
            description: 'Conceptos clave o información importante que no encaja en una tabla.',
            items: keyConceptSchema
        },
        crucialFact: { type: Type.STRING, description: 'Un dato crucial o "perla" sobre el tema.' },
        mnemonics: {
            type: Type.ARRAY,
            description: 'Reglas mnemotécnicas creativas para ayudar a recordar el concepto.',
            items: { type: Type.STRING }
        },
        reference: { type: Type.STRING, description: 'La referencia bibliográfica si se menciona en el texto.' },
        ankiNotes: {
            type: Type.ARRAY,
            description: 'Notas y consejos para crear la tarjeta en Anki, como "Errores frecuentes" o "Sugerencias visuales".',
            items: keyConceptSchema
        }
    },
...
const relatedQuestionsSchema = {
    ...
    items: {
        ...
        properties: {
            ...
            answer: { type: Type.STRING, description: "La respuesta concisa a la pregunta." },
        },
...
export const generateFlashcard = async (
    ...
    switch(depth) {
        case 'simple':
            ...
            `;
            requiredFields = ['questionStem', 'correctOption', 'definition', 'optionsAnalysis'];
            break;
        case 'deep':
             promptInstructions = `
            ...
            `;
            requiredFields = ['questionStem', 'correctOption', 'definition', 'optionsAnalysis', 'keyConcepts', 'crucialFact', 'mnemonics', 'ankiNotes'];
            break;
        case 'standard':
        default:
             promptInstructions = `
            ...
            `;
            requiredFields = ['questionStem', 'correctOption', 'definition', 'optionsAnalysis', 'keyConcepts', 'crucialFact'];
            break;
    }
...
export const generateRelatedQuestions = async (topic: string, definition: string): Promise<RelatedQuestion[]> => {
    const prompt = `
    ...
    try {
        ...
        console.error("Error al generar preguntas relacionadas:", error);
         if (error instanceof Error) {
            throw new Error(`Error de la API de Gemini al generar preguntas relacionadas: ${error.message}`);
        }
        throw new Error("Ocurrió un error desconocido al generar preguntas relacionadas.");
    }
};