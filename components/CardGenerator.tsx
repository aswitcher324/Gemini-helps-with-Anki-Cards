      ...
      <div className="bg-slate-800 p-4 rounded-xl shadow-lg border border-slate-700">
        ...
        {imagePreview && (
          <div className="mt-4 relative w-40 h-40 group animate-fade-in">
            <img
              ...
              className="rounded-lg w-full h-full object-cover border-2 border-slate-700"
            />
            <button
        ...
        <div className="mt-3 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-stretch gap-2 w-full sm:w-auto">
            <label
              className="flex items-center justify-center gap-2 cursor-pointer px-4 py-2 bg-slate-700 text-slate-200 font-semibold rounded-lg shadow-sm hover:bg-slate-600 transition-all duration-200"
              title="Añadir imagen"
            >
        ...
        <p className="text-xs text-slate-500 mt-3 text-center sm:text-left">
          Atajos: <kbd className="font-sans font-semibold">Ctrl</kbd> +{' '}
          <kbd className="font-sans font-semibold">Enter</kbd> para generar,{' '}
          <kbd className="font-sans font-semibold">Ctrl</kbd> +{' '}
          <kbd className="font-sans font-semibold">V</kbd> para pegar imagen.
        </p>
      </div>
      ...
      <div className="mt-4 min-h-[400px]">
          ...
          <div className="flex justify-center items-center h-full">
            <div className="text-center">
              ...
              <p className="text-slate-400 mt-4 text-lg">
                Analizando y estructurando tu contenido...
              </p>
            </div>
          </div>
        )}
        {error && (
          <div className="bg-red-900/50 border border-red-700 text-red-300 p-4 rounded-lg text-center">
            <h3 className="font-bold text-lg">¡Ups! Algo salió mal</h3>
            <p>{error}</p>
          </div>
        )}
        {!isLoading && !error && cardData && <Flashcard data={cardData} />}
        {!isLoading && !error && !cardData && <WelcomePlaceholder />}
      </div>
    </div>
  );
}
