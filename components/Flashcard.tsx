...
import { CheckCircle2, XCircle, Target, BookOpen, Brain, Lightbulb, FileText, Bot, Table2, Layers, ClipboardCopy, Check, HelpCircle } from 'lucide-react';
...
function generateAnkiAnswerHtml(data: FlashcardData): string {
    ...
    if (data.analysisTables && data.analysisTables.length > 0) {
        ...
        data.analysisTables.forEach(table => {
            ...
            body += '<table><thead><tr>';
            table.headers.forEach(header => { body += `<th>${header}</th>`; });
            body += '</tr></thead><tbody>';
    ...
    if (data.keyConcepts && data.keyConcepts.length > 0) {
        ...
        data.keyConcepts.forEach(concept => {
            ...
            concept.points.forEach(point => { body += `<li>${point}</li>`; });
            body += '</ul></div>';
        });
    }
    ...
    if (data.ankiNotes && data.ankiNotes.length > 0) {
        ...
        data.ankiNotes.forEach(note => {
            ...
            note.points.forEach(point => { body += `<li>${point}</li>`; });
            body += '</ul></div>';
        });
    }
...
function generateAnkiAnswerText(data: FlashcardData): string {
    ...
    if (data.keyConcepts && data.keyConcepts.length > 0) {
        ...
        data.keyConcepts.forEach(concept => {
            text += `\n--- ${concept.title} ---\n`;
            concept.points.forEach(point => { text += `- ${point}\n`; });
        });
        text += '\n';
    }
    ...
    if(data.mnemonics && data.mnemonics.length > 0) {
        text += '🧠 MNEMOTECNIA\n';
        data.mnemonics.forEach(mnemonic => { text += `- "${mnemonic}"\n`; });
        text += '\n';
    }
    ...
    if (data.ankiNotes && data.ankiNotes.length > 0) {
        ...
        data.ankiNotes.forEach(note => {
            text += `\n--- ${note.title} ---\n`;
            note.points.forEach(point => { text += `- ${point}\n`; });
        });
        text += '\n';
    }
...
export function Flashcard({ data }: FlashcardProps) {
    const [copyState, setCopyState] = useState<'question' | 'answer' | null>(null);
    const [relatedQuestions, setRelatedQuestions] = useState<RelatedQuestion[] | null>(null);
    ...
    const handleGenerateRelated = useCallback(async () => {
        ...
        try {
            const questions = await generateRelatedQuestions(data.questionStem, data.definition);
            setRelatedQuestions(questions);
        } catch (e) {
            if (e instanceof Error) {
                setRelatedError(e.message);
            } else {
                setRelatedError('Ocurrió un error inesperado al generar las preguntas.');
            }
        } finally {
    ...
    const handleCopy = async (type: 'question' | 'answer') => {
        try {
            if (type === 'question') {
                ...
                const plainTextOptions = optionsAnalysis.map((opt, index) => `${index + 1}. ${opt.option}`);
                ...
                const listItemsHtml = optionsAnalysis.map(opt => `<li>${opt.option}</li>`).join('');
            ...
            alert('No se pudo copiar el contenido. Asegúrate de que tu navegador lo permita y que te encuentras en un contexto seguro (https).');
        }
    };
    ...
    return (
        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 sm:p-8 shadow-2xl animate-fade-in">
            ...
            <div className="bg-slate-900/50 p-4 rounded-lg mb-6 flex flex-col sm:flex-row gap-3 justify-center border border-slate-700">
                ...
                >
                    {copyState === 'question' ? <Check className="w-5 h-5 text-green-400" /> : <ClipboardCopy className="w-5 h-5" />}
                    {copyState === 'question' ? 'Pregunta Copiada' : 'Copiar Pregunta (Anverso)'}
                </button>
                 <button 
                ...
                >
                    {copyState === 'answer' ? <Check className="w-5 h-5" /> : <ClipboardCopy className="w-5 h-5" />}
                    {copyState === 'answer' ? 'Respuesta Copiada' : 'Copiar Respuesta (Reverso)'}
                </button>
            </div>

            <div className="mb-6">
                <h2 className="text-2xl font-bold text-cyan-300 mb-4">{data.questionStem}</h2>
            ...
            <Section title="📖 Explicación" icon={<BookOpen className="w-6 h-6 text-cyan-400" />}>
                <p className="text-slate-300 leading-relaxed">{data.definition}</p>
            </Section>

            {data.optionsAnalysis && data.optionsAnalysis.length > 0 && (
                <Section title="🎯 Análisis de Opciones" icon={<Target className="w-6 h-6 text-cyan-400" />}>
                    <ul className="space-y-3">
                        {data.optionsAnalysis.map((item, index) => (
                            <li key={index} className={`p-3 rounded-lg ${item.verdict === '✅' ? 'bg-green-900/50 border border-green-700' : 'bg-red-900/30 border border-red-800'}`}>
                                <div className="flex items-start gap-3">
                                    {item.verdict === '✅' ? <CheckCircle2 className="w-5 h-5 text-green-400 mt-1 flex-shrink-0" /> : <XCircle className="w-5 h-5 text-red-400 mt-1 flex-shrink-0" />}
                                    <div>
                                        <strong className="font-semibold text-slate-200">{item.option}</strong>
            ...
            {data.analysisTables && data.analysisTables.length > 0 && (
                <Section title="📊 Tablas de Análisis" icon={<Table2 className="w-6 h-6 text-cyan-400" />}>
                    <div className="space-y-6">
                        {data.analysisTables.map((table, tableIndex) => (
                            <div key={tableIndex} className="bg-slate-900/70 rounded-lg overflow-x-auto">
                                <h4 className="font-semibold text-cyan-300 mb-3 px-4 pt-4">{table.title}</h4>
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-slate-800/50">
                                        <tr>
                                            {table.headers.map((header, headIndex) => (
                                                <th key={headIndex} scope="col" className="p-3 font-semibold text-slate-300 tracking-wider">
                                                    {header}
                                                </th>
                                            ))}
                                        </tr>
            ...
            {data.keyConcepts && data.keyConcepts.length > 0 && (
                <Section title="🔑 Conceptos Clave" icon={<Brain className="w-6 h-6 text-cyan-400" />}>
                    <div className="space-y-4">
                        {data.keyConcepts.map((concept, index) => (
                            <div key={index} className="bg-slate-900/70 p-4 rounded-lg">
                                <h4 className="font-semibold text-cyan-300 mb-2">{concept.title}</h4>
            ...
            {data.crucialFact && (
                <Section title="💡 Dato Crucial" icon={<Lightbulb className="w-6 h-6 text-cyan-400" />}>
                     <div className="bg-yellow-900/30 border border-yellow-700 p-4 rounded-lg">
                        <p className="text-yellow-300 leading-relaxed">🌟 {data.crucialFact}</p>
                     </div>
                </Section>
            )}

            {data.mnemonics && data.mnemonics.length > 0 && (
                <Section title="🧠 Mnemotecnia" icon={<Bot className="w-6 h-6 text-cyan-400" />}>
                    <div className="space-y-3">
                        {data.mnemonics.map((mnemonic, index) => (
                            <p key={index} className="text-slate-300 italic bg-slate-900/50 p-3 rounded-md">"{mnemonic}"</p>
                        ))}
                    </div>
                </Section>
            )}
            ...
            {data.ankiNotes && data.ankiNotes.length > 0 && (
                <Section title="📝 Notas para Anki" icon={<Layers className="w-6 h-6 text-cyan-400" />}>
                    <div className="space-y-4">
                        {data.ankiNotes.map((note, index) => (
                            <div key={index} className="bg-slate-900/70 p-4 rounded-lg">
                                <h4 className="font-semibold text-cyan-300 mb-2">{note.title}</h4>
                                <ul className="list-disc list-inside space-y-1 text-slate-300">
             ...
             {data.reference && (
                <Section title="📚 Referencia" icon={<FileText className="w-6 h-6 text-cyan-400" />}>
                    <p className="text-slate-400 text-sm italic">{data.reference}</p>
                </Section>
            )}

            <Section title="🧠 Refuerza tu Conocimiento" icon={<HelpCircle className="w-6 h-6 text-cyan-400" />}>
                {!relatedQuestions && !isGeneratingRelated && !relatedError && (
                     <div className="text-center bg-slate-900/50 p-6 rounded-lg border border-slate-700">
                        <p className="text-slate-300 mb-4">¿Has entendido bien el concepto? Ponte a prueba con preguntas relacionadas.</p>
                        <button
                ...
                {relatedQuestions && (
                    <div className="space-y-3">
                        {relatedQuestions.map((q, index) => (
                           <RelatedQuestionItem key={index} question={q.question} answer={q.answer} />
                        ))}
                    </div>
                )}
            </Section>
